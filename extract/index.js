...
const extractAllExpressions = (node) => {
    ...
    if ((_b = (_a = node.expression) === null || _a === void 0 ? void 0 : _a.expressions) === null || _b === void 0 ? void 0 : _b.length) {
        for (const exp of (_c = node.expression) === null || _c === void 0 ? void 0 : _c.expressions) {
...
async function findAppModules() {
    ...
    const result = qrModules.filter(m => {
        const hasProto = !!m.value.body.body.find(b => {
            ...
            return expressions === null || expressions === void 0 ? void 0 : expressions.find(e => { var _a, _b; return ((_b = (_a = e === null || e === void 0 ? void 0 : e.left) === null || _a === void 0 ? void 0 : _a.property) === null || _b === void 0 ? void 0 : _b.name) === 'internalSpec'; });
        });
        if (hasProto) {
            return true;
        }
...
(async () => {
    ...
    for (const mod of modules) {
        ...
        walk.simple(mod, {
            VariableDeclarator(node) {
                var _a;
                if (((_a = node.init) === null || _a === void 0 ? void 0 : _a.type) === 'CallExpression'
                    ...
                    members = members.map(({ key: { name }, value: { elements } }) => {
                        ...
                        if (type === 'message' || type === 'enum') {
                            ...
                            if (elements[2].type === 'Identifier') {
                                type = (_a = Object.values(modInfo.identifiers).find(v => v.alias === elements[2].name)) === null || _a === void 0 ? void 0 : _a.name;
                                if (!type) {
    ...
    for (const mod of modules) {
        ...
        const stringifyEnum = (ident, overrideName = null) => [].concat([`enum ${overrideName || ident.displayName || ident.name} {`], addPrefix(ident.enumValues.map(v => `${v.name} = ${v.id};`), spaceIndent), ['}']);
        ...
        const stringifyMessageSpecMember = (info, completeFlags, parentName = undefined) => {
            ...
            if (info.type === '__oneof__') {
                return [].concat([`oneof ${info.name} {`], addPrefix([].concat(...info.members.map(m => stringifyMessageSpecMember(m, false))), spaceIndent), ['}']);
            }
            else {
                ...
                const ret = [];
                const indentation = (_a = moduleIndentationMap[info.type]) === null || _a === void 0 ? void 0 : _a.indentation;
        ...
        const stringifyMessageSpec = (ident) => {
            var _a;
            const members = (_a = moduleIndentationMap[ident.name]) === null || _a === void 0 ? void 0 : _a.members;
            const result = [];
            result.push(`message ${ident.displayName || ident.name} {`, ...addPrefix([].concat(...ident.members.map(m => stringifyMessageSpecMember(m, true, ident.name))), spaceIndent));
            if (members === null || members === void 0 ? void 0 : members.size) {
        ...
        const getEntity = (v) => {
            ...
            else if ((_a = v.enumValues) === null || _a === void 0 ? void 0 : _a.length) {
        ...
        for (const value of identifiers) {
            ...
            if (!((_b = (_a = moduleIndentationMap[name]) === null || _a === void 0 ? void 0 : _a.indentation) === null || _b === void 0 ? void 0 : _b.length)) {
