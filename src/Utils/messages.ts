...
import { MEDIA_KEYS, URL_REGEX, WA_DEFAULT_EPHEMERAL } from '../Base'
import { isJidGroup, isJidStatusBroadcast, jidNormalizedUser } from '../Binary'
...
export const getContentType = (content: WAProto.IMessage | undefined) => {
	if(content) {
		...
		const key = keys.find(
			k => (k === 'conversation' || k.includes('Message')) &&
 ...
 */
export const getDevice = (id: string) => /^3A.{18}$/.test(id)
	? 'ios'
	: /^3E.{20}$/.test(id)
		? 'web'
		: /^(.{21}|.{32})$/.test(id)
