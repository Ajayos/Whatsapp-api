...
function append(data, historyCache, event, eventData, logger) {
	...
	switch (event) {
		...
		case 'messages.upsert':
			...
			for (const message of messages) {
				const key = stringifyMessageKey(message.key);
				let existing =
					(_a = data.messageUpserts[key]) === null || _a === void 0
						? void 0
						: _a.message;
				if (!existing) {
		...
		case 'message-receipt.update':
			...
			for (const { key, receipt } of receipts) {
				...
				if (existing) {
					...
					(0, messages_1.updateMessageWithReceipt)(
						data.messageReceipts[keyStr],
						receipt,
					);
				}
			}
	...
	function decrementChatReadCounterIfMsgDidUnread(message) {
		...
		if (
			...
			typeof (chat === null || chat === void 0 ? void 0 : chat.unreadCount) ===
				'number' &&
			chat.unreadCount > 0
		) {
