...
const downsample_1 = require("downsample");
...
const hkdfInfoKey = (type) => {
    const hkdfInfo = Base_1.MEDIA_HKDF_KEY_MAPPING[type];
...
async function getAudioWaveform(buffer) {
    const oggDecoderModule = await (0, tsimportlib_1.dynamicImport)('ogg-opus-decoder', module);
    const decoder = new oggDecoderModule.OggOpusDecoder();
    await decoder.ready;
    let audioInfo = undefined;
    if (Buffer.isBuffer(buffer)) {
        audioInfo = await decoder.decodeFile(buffer);
    }
    else if (typeof buffer === 'string') {
        const rStream = (0, fs_1.createReadStream)(buffer);
        const readAllChunks = new Promise((resolve, reject) => {
    ...
    const simpleMovingAverage = (values, window = 5, arrayAverage) => {
    ...
    if (audioInfo && audioInfo.channelData.length >= 1) {
        const wave = [...audioInfo.channelData[0].map((a) => a).filter((a) => a >= 0)];
        const ampDownsampled = [];
        const downsampled = (0, downsample_1.LTTB)(wave.map((value, index) => [index, Math.round(value * 100)]), 64);
...
const decodeMediaRetryNode = (node) => {
    ...
    if (errorNode) {
        ...
        const ciphertext = (0, Binary_1.getBinaryNodeChildBuffer)(encryptedInfoNode, 'enc_p');
        const iv = (0, Binary_1.getBinaryNodeChildBuffer)(encryptedInfoNode, 'enc_iv');
        if (ciphertext && iv) {
