...
const decodePatches = async (
	...
	for (let i = 0; i < syncds.length; i++) {
		...
		if (syncd.externalMutations) {
			...
			const ref = await (0, exports.downloadExternalPatch)(
			...
			(_a = syncd.mutations) === null || _a === void 0
				? void 0
				: _a.push(...ref.mutations);
		}
...
const chatModificationToAppPatch = (mod, jid) => {
		...
		if (Array.isArray(lastMessages)) {
			...
			messageRange = {
				lastMessageTimestamp:
					lastMsg === null || lastMsg === void 0
						? void 0
						: lastMsg.messageTimestamp,
				messages: (
					lastMessages === null || lastMessages === void 0
						? void 0
						: lastMessages.length
				)
					? lastMessages.map(m => {
	...
	if ('mute' in mod) {
		...
		patch = {
			...
			index: [
				LabelAssociation_1.LabelAssociationType.Chat,
				mod.addChatLabel.labelId,
				jid,
			],
			type: 'regular',
			apiVersion: 3,
...
const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
	...
	if (action === null || action === void 0 ? void 0 : action.muteAction) {
		...
		action === null || action === void 0 ? void 0 : action.markChatAsReadAction
	) {
		...
		action === null || action === void 0 ? void 0 : action.contactAction
	) {
		ev.emit('contacts.upsert', [{ id, name: action.contactAction.fullName }]);
	} else if (
		action === null || action === void 0 ? void 0 : action.pushNameSetting
	) {
		const name =
			(_b =
				action === null || action === void 0
					? void 0
					: action.pushNameSetting) === null || _b === void 0
				? void 0
				: _b.name;
		...
		ev.emit('chats.update', [
			{
				id,
				pinned: (
					(_c = action.pinAction) === null || _c === void 0 ? void 0 : _c.pinned
				)
		...
		action === null || action === void 0 ? void 0 : action.unarchiveChatsSetting
	) {
		...
		action === null || action === void 0 ? void 0 : action.labelEditAction
	) {
	...
	function isValidPatchBasedOnMessageRange(chat, msgRange) {
		...
		const chatLastMsgTimestamp = Number(
			(chat === null || chat === void 0
				? void 0
				: chat.lastMessageRecvTimestamp) || 0,
		);
