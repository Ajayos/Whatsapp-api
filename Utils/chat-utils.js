...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        const operation = 'operation' in msgMutation ? msgMutation.operation : Proto_1.proto.SyncdMutation.SyncdOperation.SET;
...
const extractSyncdPatches = async (result, options) => {
    ...
    const collectionNodes = (0, Binary_1.getBinaryNodeChildren)(syncNode, 'collection');
    const final = {};
    await Promise.all(collectionNodes.map(async (collectionNode) => {
        const patchesNode = (0, Binary_1.getBinaryNodeChild)(collectionNode, 'patches');
        const patches = (0, Binary_1.getBinaryNodeChildren)(patchesNode || collectionNode, 'patch');
        const snapshotNode = (0, Binary_1.getBinaryNodeChild)(collectionNode, 'snapshot');
        const syncds = [];
        ...
        if (snapshotNode && !!snapshotNode.content) {
            ...
            const blobRef = Proto_1.proto.ExternalBlobReference.decode(snapshotNode.content);
        ...
        if (Array.isArray(lastMessages)) {
            ...
            messageRange = {
                ...
                messages: (lastMessages === null || lastMessages === void 0 ? void 0 : lastMessages.length) ? lastMessages.map(m => {
                    ...
                    if ((0, Binary_1.isJidGroup)(m.key.remoteJid) && !m.key.fromMe && !m.key.participant) {
                    ...
                    if (m.key.participant) {
                        m.key.participant = (0, Binary_1.jidNormalizedUser)(m.key.participant);
                    }
                    return m;
                }) : undefined
            };
