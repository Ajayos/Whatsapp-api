...
const makeSocket = (config) => {
    var _a, _b;
    const { connectTimeoutMs, logger, keepAliveIntervalMs, browser, auth: authState, printQRInTerminal, defaultQueryTimeoutMs, transactionOpts, qrTimeout, makeSignalRepository, } = config;
    let url = new url_1.URL('wss://web.whatsapp.com/ws/chat');
    config.mobile = config.mobile || url.protocol === 'tcp:';
    if (config.mobile && url.protocol !== 'tcp:') {
        url = new url_1.URL(`tcp://${Base_1.MOBILE_ENDPOINT}:${Base_1.MOBILE_PORT}`);
    }
    if (!config.mobile &&
        url.protocol === 'wss' &&
    ...
    const noise = (0, Utils_1.makeNoiseHandler)({
        ...
        NOISE_HEADER: config.mobile ? Base_1.MOBILE_NOISE_HEADER : Base_1.NOISE_WA_HEADER,
    ...
    const sendNode = (frame) => {
        if (logger.level === 'trace') {
            logger.trace({ xml: (0, Binary_1.binaryNodeToString)(frame), msg: 'xml send' });
        }
    ...
    const validateConnection = async () => {
        ...
        const payloadEnc = noise.encrypt(Proto_1.proto.ClientPayload.encode(node).finish());
    ...
    const uploadPreKeys = async (count = Base_1.INITIAL_PREKEY_COUNT) => {
    ...
    const uploadPreKeysToServerIfRequired = async () => {
        ...
        if (preKeyCount <= Base_1.MIN_PREKEY_COUNT) {
            await uploadPreKeys();
        }
    };
    const onMessageReceived = (data) => {
        noise.decodeFrame(data, frame => {
            ...
            if (!(frame instanceof Uint8Array)) {
                ...
                if (logger.level === 'trace') {
                    logger.trace({ xml: (0, Binary_1.binaryNodeToString)(frame), msg: 'recv xml' });
                }
                /* Check if this is a response to a message we sent */
                anyTriggered =
                    ws.emit(`${Base_1.DEF_TAG_PREFIX}${msgId}`, frame) || anyTriggered;
                ...
                Object.keys(l1).forEach(key => {
                    anyTriggered =
                        ws.emit(`${Base_1.DEF_CALLBACK_PREFIX}${l0},${key}:${l1[key]},${l2}`, frame) || anyTriggered;
                    anyTriggered =
                        ws.emit(`${Base_1.DEF_CALLBACK_PREFIX}${l0},${key}:${l1[key]}`, frame) ||
                            anyTriggered;
                    anyTriggered =
                ...
                anyTriggered =
                    ws.emit(`${Base_1.DEF_CALLBACK_PREFIX}${l0},,${l2}`, frame) || anyTriggered;
                anyTriggered =
                    ws.emit(`${Base_1.DEF_CALLBACK_PREFIX}${l0}`, frame) || anyTriggered;
    ...
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        await sendNode(iq);
        const pairDeviceNode = (0, Binary_1.getBinaryNodeChild)(stanza, 'pair-device');
    ...
    ws.on('CB:ib,,edge_routing', (node) => {
        const edgeRoutingNode = (0, Binary_1.getBinaryNodeChild)(node, 'edge_routing');
        const routingInfo = (0, Binary_1.getBinaryNodeChild)(edgeRoutingNode, 'routing_info');
