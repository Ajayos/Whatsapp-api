...
const parseCatalogNode = (node) => {
    ...
    const products = (0, Binary_1.getBinaryNodeChildren)(catalogNode, "product").map(exports.parseProductNode);
...
const parseCollectionsNode = (node) => {
    ...
    const collections = (0, Binary_1.getBinaryNodeChildren)(collectionsNode, "collection").map((collectionNode) => {
        ...
        const products = (0, Binary_1.getBinaryNodeChildren)(collectionNode, "product").map(exports.parseProductNode);
        return {
            id,
            name,
            products,
...
const parseOrderDetailsNode = (node) => {
    ...
    const products = (0, Binary_1.getBinaryNodeChildren)(orderNode, "product").map((productNode) => {
        ...
        return {
            ...
            quantity: +(0, Binary_1.getBinaryNodeChildString)(productNode, "quantity"),
        };
    });
...
const parseProductNode = (productNode) => {
    ...
    const statusInfoNode = (0, Binary_1.getBinaryNodeChild)(productNode, "status_info");
    const product = {
        id,
        imageUrls: parseImageUrls(mediaNode),
        reviewStatus: {
            whatsapp: (0, Binary_1.getBinaryNodeChildString)(statusInfoNode, "status"),
        },
        availability: "in stock",
        name: (0, Binary_1.getBinaryNodeChildString)(productNode, "name"),
        retailerId: (0, Binary_1.getBinaryNodeChildString)(productNode, "retailer_id"),
        url: (0, Binary_1.getBinaryNodeChildString)(productNode, "url"),
        description: (0, Binary_1.getBinaryNodeChildString)(productNode, "description"),
        price: +(0, Binary_1.getBinaryNodeChildString)(productNode, "price"),
        currency: (0, Binary_1.getBinaryNodeChildString)(productNode, "currency"),
        isHidden,
    };
...
const parseImageUrls = (mediaNode) => {
    ...
    return {
        requested: (0, Binary_1.getBinaryNodeChildString)(imgNode, "request_image_url"),
        original: (0, Binary_1.getBinaryNodeChildString)(imgNode, "original_image_url"),
    };
};
const parseStatusInfo = (mediaNode) => {
    ...
    return {
        ...
        canAppeal: (0, Binary_1.getBinaryNodeChildString)(node, "can_appeal") === "true",
    };
};
