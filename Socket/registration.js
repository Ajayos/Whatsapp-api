...
const makeRegistrationSocket = (config) => {
    ...
    const register = async (code) => {
        ...
        sock.authState.creds.me = {
            id: (0, Binary_1.jidEncode)(result.login, 's.whatsapp.net'),
            name: '~'
        };
...
function registrationParams(params) {
    ...
    return {
        ...
        token: (0, crypto_1.md5)(Buffer.concat([Base_1.MOBILE_TOKEN, Buffer.from(params.phoneNumberNationalNumber)])).toString('hex'),
    };
}
...
function mobileRegisterEncrypt(data) {
    ...
    const key = crypto_1.Curve.sharedKey(keypair.private, Base_1.REGISTRATION_PUBLIC_KEY);
...
async function mobileRegisterFetch(path, opts = {}) {
    ...
    opts.headers['User-Agent'] = Base_1.MOBILE_USERAGENT;
