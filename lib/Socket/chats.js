...
const makeChatsSocket = (config) => {
    ...
    const fetchBlocklist = async () => {
        ...
        return (0, Binary_1.getBinaryNodeChildren)(listNode, 'item')
            .map(n => n.attrs.jid);
    };
    ...
    const getBusinessProfile = async (jid) => {
        ...
        if (profiles) {
            ...
            const business_hours = (0, Binary_1.getBinaryNodeChild)(profiles, 'business_hours');
            const business_hours_config = business_hours && (0, Binary_1.getBinaryNodeChildren)(business_hours, 'business_hours_config');
    ...
    const handlePresenceUpdate = ({ tag, attrs, content }) => {
        ...
        if (presence) {
            ev.emit('presence.update', { id: jid, presences: presence, participant: participant });
        }
    };
    ...
    const fetchAbt = async () => {
        ...
        const propsNode = (0, Binary_1.getBinaryNodeChild)(abtNode, 'props');
        let props = {};
        if (propsNode) {
