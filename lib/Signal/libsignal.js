...
function makeLibSignalRepository(auth) {
    ...
    return {
        ...
        async processSenderKeyDistributionMessage({ item, authorJid }) {
            ...
            const senderMsg = new SignalGroup_1.SenderKeyDistributionMessage(null, null, null, null, item.axolotlSenderKeyDistributionMessage);
            const { [senderName]: senderKey } = await auth.keys.get('sender-key', [senderName]);
            if (!senderKey) {
                await storage.storeSenderKey(senderName, new SignalGroup_1.SenderKeyRecord());
            }
...
const jidToSignalSenderKeyName = (group, user) => {
    return new SignalGroup_1.SenderKeyName(group, jidToSignalProtocolAddress(user)).toString();
};
