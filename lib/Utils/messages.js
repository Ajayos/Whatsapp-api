...
const extractUrlFromText = (text) => {
    var _a;
    return (!Base_1.URL_EXCLUDE_REGEX.test(text) ? (_a = text.match(Base_1.URL_REGEX)) === null || _a === void 0 ? void 0 : _a[0] : undefined);
};
...
const prepareWAMessageMedia = async (message, options) => {
    ...
    let mediaType;
    for (const key of Base_1.MEDIA_KEYS) {
        if (key in message) {
            mediaType = key;
        }
    }
...
const generateForwardMessageContent = (message, forceForward) => {
    ...
    content = Proto_1.proto.Message.decode(Proto_1.proto.Message.encode(content).finish());
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        const exp = typeof message.disappearingMessagesInChat === 'boolean' ?
            (message.disappearingMessagesInChat ? Base_1.WA_DEFAULT_EPHEMERAL : 0) :
    ...
    if ('buttons' in message && !!message.buttons) {
        const buttonsMessage = {
            buttons: message.buttons.map(b => ({ ...b, type: Proto_1.proto.Message.ButtonsMessage.Button.Type.RESPONSE }))
        };
        if ('text' in message) {
...
const generateWAMessageFromContent = (jid, message, options) => {
    ...
    if (quoted) {
        ...
        quotedMsg = Proto_1.proto.Message.fromObject({ [msgType]: quotedMsg[msgType] });
    ...
    !!(options === null || options === void 0 ? void 0 : options.ephemeralExpiration) &&
        ...
        message[key].contextInfo = {
            ...(message[key].contextInfo || {}),
            expiration: options.ephemeralExpiration || Base_1.WA_DEFAULT_EPHEMERAL,
    ...
    const messageJSON = {
        ...
        participant: (0, Binary_1.isJidGroup)(jid) ? userJid : undefined,
